...
class DetectThread(Thread):
    """! The LoRaWAN object utilizes the RB2483A transceiver from Microchip
    ...
    def _read(self: Thread) -> str:
        """! Used to read from the serial interface of the transceiver
        @return     Returns the resulting response
        """
        tmp = self._ser.readline().decode("UTF-8").strip()
    ...
    def initialize(self: Thread):
        ...
        # Instead of 99% off, the channels have a (100% - (1 / channel-count)) dty-cycle
    ...
    def run(self: Thread) -> None:
        ...
        # Ensure the transceiver is initialized
        try:
            self.initialize()
        ...
        while not self.stopped:
            ...
            # Convert monitoring results in the string
            data_bin_str = ""
    ...
    def stop(self: Thread) -> None:
        """! Stops the LoRaWAN Thread and joins it
        """
        self.stopped = True
        self.join()
